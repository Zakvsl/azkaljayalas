...
<div class="container mx-auto px-4 py-6">
    <!-- Header -->
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900">Edit Data Training</h1>
        <p class="mt-2 text-sm text-gray-600">Update data training untuk model ML</p>
    </div>

    <!-- Form Card -->
    <div class="bg-white shadow rounded-lg">
        <form action="{{ route('admin.training-data.update', $trainingDatum->id) }}" method="POST" class="p-6">
            @csrf
            @method('PATCH')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="produk" id="produk" required
                        ...
                        <option value="Pagar" {{ old('produk', $trainingDatum->produk) == 'Pagar' ? 'selected' : '' }}>Pagar</option>
                        <option value="Kanopi" {{ old('produk', $trainingDatum->produk) == 'Kanopi' ? 'selected' : '' }}>Kanopi</option>
                        <option value="Railing" {{ old('produk', $trainingDatum->produk) == 'Railing' ? 'selected' : '' }}>Railing</option>
                        <option value="Teralis" {{ old('produk', $trainingDatum->produk) == 'Teralis' ? 'selected' : '' }}>Teralis</option>
                        <option value="Pintu" {{ old('produk', $trainingDatum->produk) == 'Pintu' ? 'selected' : '' }}>Pintu</option>
                    </select>
                    @error('produk')
                ...
                <div>
                    ...
                    <input type="number" name="jumlah_unit" id="jumlah_unit" value="{{ old('jumlah_unit', $trainingDatum->jumlah_unit) }}" 
                           min="1" required
                ...
                <div>
                    ...
                    <select name="metode_hitung" id="metode_hitung" required
                        ...
                        <option value="Per m²" {{ old('metode_hitung', $trainingDatum->metode_hitung) == 'Per m²' ? 'selected' : '' }}>Per m²</option>
                        <option value="Per Lubang" {{ old('metode_hitung', $trainingDatum->metode_hitung) == 'Per Lubang' ? 'selected' : '' }}>Per Lubang</option>
                    </select>
                    @error('metode_hitung')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div id="ukuran_m2_wrapper">
                    ...
                    <input type="number" name="ukuran_m2" id="ukuran_m2" value="{{ old('ukuran_m2', $trainingDatum->ukuran_m2) }}" 
                           step="0.01" min="0"
                ...
                <div id="jumlah_lubang_wrapper">
                    ...
                    <input type="number" name="jumlah_lubang" id="jumlah_lubang" value="{{ old('jumlah_lubang', $trainingDatum->jumlah_lubang) }}" 
                           step="0.1" min="0"
                ...
                <div>
                    ...
                    <select name="jenis_material" id="jenis_material" required
                        ...
                        <option value="Hollow" {{ old('jenis_material', $trainingDatum->jenis_material) == 'Hollow' ? 'selected' : '' }}>Hollow</option>
                        <option value="Besi" {{ old('jenis_material', $trainingDatum->jenis_material) == 'Besi' ? 'selected' : '' }}>Besi</option>
                        <option value="Stainless" {{ old('jenis_material', $trainingDatum->jenis_material) == 'Stainless' ? 'selected' : '' }}>Stainless</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="ketebalan_mm" id="ketebalan_mm" value="{{ old('ketebalan_mm', $trainingDatum->ketebalan_mm) }}" 
                ...
                <div>
                    ...
                    <select name="finishing" id="finishing" required
                        ...
                        <option value="Cat" {{ old('finishing', $trainingDatum->finishing) == 'Cat' ? 'selected' : '' }}>Cat</option>
                        <option value="Powder Coating" {{ old('finishing', $trainingDatum->finishing) == 'Powder Coating' ? 'selected' : '' }}>Powder Coating</option>
                        <option value="Tanpa Finishing" {{ old('finishing', $trainingDatum->finishing) == 'Tanpa Finishing' ? 'selected' : '' }}>Tanpa Finishing</option>
                    </select>
                    @error('finishing')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="kerumitan_desain" id="kerumitan_desain" required
                        ...
                        <option value="Sederhana" {{ old('kerumitan_desain', $trainingDatum->kerumitan_desain) == 'Sederhana' ? 'selected' : '' }}>Sederhana</option>
                        <option value="Menengah" {{ old('kerumitan_desain', $trainingDatum->kerumitan_desain) == 'Menengah' ? 'selected' : '' }}>Menengah</option>
                        <option value="Kompleks" {{ old('kerumitan_desain', $trainingDatum->kerumitan_desain) == 'Kompleks' ? 'selected' : '' }}>Kompleks</option>
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="number" name="harga_akhir" id="harga_akhir" value="{{ old('harga_akhir', $trainingDatum->harga_akhir) }}" 
                           min="0" required
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="notes" id="notes" rows="3"
                              class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('notes') border-red-500 @enderror">{{ old('notes', $trainingDatum->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mt-6 flex justify-end space-x-3">
                <a href="{{ route('admin.training-data.show', $trainingDatum->id) }}" 
